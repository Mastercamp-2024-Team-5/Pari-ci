...
const MapScreen = () => {
  ...
  useEffect(() => {
    const fetchStops = async () => {
      try {
        ...
        if (
          !Array.isArray(metroData) ||
          !metroData.length ||
          !Array.isArray(rerData) ||
          !rerData.length
        ) {
        ...
        const combinedData = [
          ...metroData,
          ...rerData.filter((stop) => colors[stop.route_id] !== undefined),
        ];
  ...
  function setlines(stops: Stop[]) {
    ...
    for (let i = 0; i < Object.keys(colors).length; i++) {
      const line = stops.filter(
        (stop) => stop.route_id === Object.keys(colors)[i]
      );
      if (line.length > 0) {
        const lineFeatures: GeoJSON.Feature<GeoJSON.LineString> = {
          ...
          geometry: {
            type: "LineString",
            coordinates: line.map((stop: Stop) => [
              stop.stop_lon,
              stop.stop_lat,
